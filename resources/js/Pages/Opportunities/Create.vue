...
<template>
    <div class="max-w-2xl">
        <div class="flex items-center gap-3 mb-6">
            ...
            <h1 class="text-xl font-bold text-gray-900 dark:text-white">Nouvelle opportunite</h1>
        </div>
        ...
        <div class="panel">
            <form @submit.prevent="submit" class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <Select v-model="form.pipeline_stage_id" :options="stages" optionLabel="name" optionValue="id" placeholder="Selectionner..." class="w-full" :invalid="!!form.errors.pipeline_stage_id" />
                    <small v-if="form.errors.pipeline_stage_id" class="text-rose-500 mt-1">{{ form.errors.pipeline_stage_id }}</small>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Entreprise</label>
                        <Select v-model="form.company_id" :options="companies" optionLabel="name" optionValue="id" placeholder="Selectionner..." showClear filter class="w-full" @change="form.contact_id = null" />
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Contact</label>
                        <Select v-model="form.contact_id" :options="filteredContacts" optionLabel="name" optionValue="id" placeholder="Selectionner..." showClear filter class="w-full" />
                    </div>
                </div>
                <div>
