...
<template>
    <div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <h1 class="text-2xl font-bold text-surface-900 dark:text-surface-0">Tâches</h1>
            <div class="flex gap-2">
                <Link href="/app/tasks/my-day">
                    <Button label="Ma Journée" icon="pi pi-calendar" severity="secondary" outlined />
                </Link>
                <Link href="/app/tasks/create">
                    <Button label="Nouvelle tâche" icon="pi pi-plus" />
                </Link>
            </div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 mb-4 flex-wrap">
            ...
            <Select v-model="priorityFilter" :options="priorityOptions" optionLabel="label" optionValue="value" placeholder="Toutes priorités" showClear class="w-full sm:w-40" />
        </div>

        <DataTable :value="tasks.data" :rows="15" stripedRows class="text-sm" :rowClass="rowClass">
            ...
            <Column field="title" header="Titre" sortable>
                <template #body="{ data }">
                    <Link :href="`/app/tasks/${data.id}/edit`" class="font-medium text-primary hover:underline">{{ data.title }}</Link>
            ...
            <Column field="due_date" header="Échéance" sortable>
                <template #body="{ data }">
                    <span :class="{ 'text-red-500 font-medium': data.is_overdue, 'text-amber-600': data.is_today }">
                        {{ formatDate(data.due_date) }}
                    </span>
                    <span v-if="data.due_time" class="block text-xs text-surface-500">{{ data.due_time }}</span>
                </template>
            </Column>
            <Column field="priority" header="Priorité" style="width: 100px">
                <template #body="{ data }">
            ...
            <Column header="Statut" style="width: 80px">
                <template #body="{ data }">
                    ...
                    <i v-else class="pi pi-circle text-surface-300"></i>
                </template>
            </Column>
            <Column header="Actions" style="width: 140px">
                <template #body="{ data }">
                    <div class="flex gap-1">
                        <Button v-if="!data.completed_at" icon="pi pi-check" text rounded size="small" severity="success" @click="openCompleteDialog(data)" v-tooltip="'Compléter'" />
            ...
            <template #empty><div class="text-center py-8 text-surface-500">Aucune tâche trouvée</div></template>
        </DataTable>

        <div v-if="tasks.links?.length > 3" class="flex justify-center gap-1 mt-4">
